class ActionEncoder
  ...
  pai_strs:["1m","2m","3m","4m","5m","6m","7m","8m","9m","1p","2p","3p","4p","5p","6p","7p","8p","9p","1s","2s","3s","4s","5s","6s","7s","8s","9s","E","S","W","N","P","F","C","5mr","5pr","5sr"]
  json_to_action:(a)->
    a = JSON.parse(a)
...
class CliantController extends ActionEncoder
  constructor:(socket)->
    ...
    @socket.onmessage = (a)=>
      @receive(@json_to_action a.data)
    
    @socket.onopen = ()=>
      console.log("connect")
      @socket.send(@action_to_json {type: "join", name: "", room: "default"})
    alert "join"
  ...
  send:(a)->
    console.log("send",@action_to_json a)
    @socket.send(@action_to_json a)
